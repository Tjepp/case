...
export class CaseRating extends LitElement {
  ...
  static styles = css`
    ...
    .container:hover > button {
      fill: var(--custom-star-active, ${defaultActive});
    }

    .container > button:hover ~ button {
      fill: var(--custom-star-inactive, ${defaultInactive});
    }

    button:focus-visible {
      outline: 1px solid ${defaultOutline};
      box-shadow: 0 0 12px ${defaultOutline};
    }
    ...
    return html`<div class="container">
        ${map(range(1, 6), (starNumber) => {
          return html`<button
            ...
            style="--star-color: ${this.rating >= starNumber
              ? `var(--custom-star-active, ${defaultActive})`
              : `var(--custom-star-inactive, ${defaultInactive})`}"
          >
